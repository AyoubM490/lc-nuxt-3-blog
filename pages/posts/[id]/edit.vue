...
onMounted(async () => {
    try {
        post.value = await $apiFetch(`/api/postsAuth/${route.params.id}`).catch(err => console.log(err))
...
<template>
    <div class="container mx-auto w-1/2 py-8">
        ...
        <div class="mt-4">
            <button class="inline-block bg-red-600 hover:bg-red-700 text-white rounded px-4 py-2" @click="deletePost">Delete Post</button>
        </div>
    </div>
</template>
